"""
Импорт пользователей и платежей из Excel (бот Region / regionsvyaz) в БД Lead Tracker.

Формат файла такой же, как у gagarin_import.xlsx (лист users + те же листы платежей).
...
На сервере с Docker (файл в корне репозитория на хосте монтируется в контейнер как /app/repo):
  ...
  docker compose exec api uv run python scripts/region_import.py /app/repo/region_import.xlsx
  docker compose exec api uv run python scripts/region_import.py --dry-run /app/repo/region_import.xlsx

Локально из корня репозитория (нужны .env и доступ к Postgres):

  uv run python scripts/region_import.py ./region_import.xlsx
  uv run python scripts/region_import.py --dry-run ./region_import.xlsx

Без аргумента пути берётся ./region_import.xlsx или /app/repo/region_import.xlsx в Docker.
...
def _default_xlsx_path() -> str:
    mounted = Path("/app/repo/region_import.xlsx")
    if mounted.parent.is_dir():
        return str(mounted)
    return str(_ROOT / "region_import.xlsx")
...
def main() -> None:
    p = argparse.ArgumentParser(description="Импорт users + payments из Excel (Region / regionsvyaz).")
    p.add_argument(
        "xlsx",
        nargs="?",
        default=_default_xlsx_path(),
        help="Путь к .xlsx (по умолчанию ./region_import.xlsx или /app/repo/region_import.xlsx в Docker)",
    )
