"""
Импорт пользователей и платежей из Excel (бот fastmobilevpnbot) в БД Lead Tracker.

Файл в корне репозитория на VPS (см. volume `api`: `.:/app/repo`) или любой путь аргументом.

  docker compose exec api uv run python scripts/social_import.py /app/repo/social_import.xlsx
  docker compose exec api uv run python scripts/social_import.py --dry-run /app/repo/social_import.xlsx

Локально (из корня репозитория, с .env и доступом к Postgres):

  uv run python scripts/social_import.py ./social_import.xlsx
...
# Запуск из корня репозитория без Docker: добавить src в PYTHONPATH
...
def _default_xlsx_path() -> str:
    """В Docker с volume `.:/app/repo` — файл из корня репозитория на хосте."""
    mounted = Path("/app/repo/social_import.xlsx")
    if mounted.parent.is_dir():
        return str(mounted)
    return str(_ROOT / "social_import.xlsx")
...
def load_users_rows(xl: pd.ExcelFile) -> list[dict]:
    ...
    # дубликаты в Excel: оставляем последнюю строку
...
def load_payment_rows(xl: pd.ExcelFile, sheet: str) -> list[dict]:
    ...
    logger.info("payments {!r}: отобрано строк: {}", sheet, len(out))
    return out
...
async def main_async(xlsx: Path) -> None:
    ...
    logger.info("Всего строк платежей (после фильтра): {}", len(pay_rows))
...
def main() -> None:
    p = argparse.ArgumentParser(description="Импорт users + payments из Excel (fastmobilevpnbot).")
    p.add_argument(
        "xlsx",
        nargs="?",
        default=_default_xlsx_path(),
        help="Путь к .xlsx (по умолчанию ./social_import.xlsx или /app/repo/social_import.xlsx в Docker)",
    )
    ...
    if args.dry_run:
        ...
        logger.info("DRY-RUN: users {}, платежей всего {}", len(u), n)
        return
