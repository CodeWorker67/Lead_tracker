"""
Импорт пользователей и платежей из Excel (бот Zoomer / zoomerskyvpn_bot) в БД Lead Tracker.

Формат файла такой же, как у region_import.xlsx (лист users + те же листы платежей).
...
На сервере с Docker (файл в корне репозитория на хосте монтируется в контейнер как /app/repo):
  ...
  docker compose exec api uv run python scripts/zoomer_import.py /app/repo/zoomer_import.xlsx
  docker compose exec api uv run python scripts/zoomer_import.py --dry-run /app/repo/zoomer_import.xlsx

Локально из корня репозитория (нужны .env и доступ к Postgres):

  uv run python scripts/zoomer_import.py ./zoomer_import.xlsx
  uv run python scripts/zoomer_import.py --dry-run ./zoomer_import.xlsx

Без аргумента пути берётся ./zoomer_import.xlsx или /app/repo/zoomer_import.xlsx в Docker.
...
def _default_xlsx_path() -> str:
    mounted = Path("/app/repo/zoomer_import.xlsx")
    if mounted.parent.is_dir():
        return str(mounted)
    return str(_ROOT / "zoomer_import.xlsx")
...
def main() -> None:
    p = argparse.ArgumentParser(description="Импорт users + payments из Excel (Zoomer / zoomerskyvpn_bot).")
    p.add_argument(
        "xlsx",
        nargs="?",
        default=_default_xlsx_path(),
        help="Путь к .xlsx (по умолчанию ./zoomer_import.xlsx или /app/repo/zoomer_import.xlsx в Docker)",
    )
