"""
Импорт пользователей и платежей из Excel (бот Gagarinsky) в БД Lead Tracker.

Файл в корне репозитория на VPS (см. volume `api`: `.:/app/repo`) или любой путь аргументом.

  docker compose exec api uv run python scripts/gagarin_import.py /app/repo/tmpuedacc7i.xlsx
  docker compose exec api uv run python scripts/gagarin_import.py --dry-run /app/repo/tmpuedacc7i.xlsx
...
def _default_xlsx_path() -> str:
    ...
    mounted = Path("/app/repo/gagarin_import.xlsx")
    if mounted.parent.is_dir():
        return str(mounted)
    return str(_ROOT / "gagarin_import.xlsx")
...
def main() -> None:
    p = argparse.ArgumentParser(description="Импорт users + payments из Excel (Gagarinsky).")
    p.add_argument(
        "xlsx",
        nargs="?",
        default=_default_xlsx_path(),
        help="Путь к .xlsx (по умолчанию ./gagarin_import.xlsx или /app/repo/gagarin_import.xlsx в Docker)",
    )
