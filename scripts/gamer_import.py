"""
Импорт пользователей и платежей из Excel (бот Fast Gamer / fastgamerbot) в БД Lead Tracker.

Формат файла такой же, как у region_import.xlsx / gagarin_import.xlsx (лист users + те же листы платежей).
...
На сервере с Docker (файл в корне репозитория на хосте монтируется в контейнер как /app/repo):
  ...
  docker compose exec api uv run python scripts/gamer_import.py /app/repo/gamer_import.xlsx
  docker compose exec api uv run python scripts/gamer_import.py --dry-run /app/repo/gamer_import.xlsx

Локально из корня репозитория (нужны .env и доступ к Postgres):

  uv run python scripts/gamer_import.py ./gamer_import.xlsx
  uv run python scripts/gamer_import.py --dry-run ./gamer_import.xlsx

Без аргумента пути берётся ./gamer_import.xlsx или /app/repo/gamer_import.xlsx в Docker.
...
def _default_xlsx_path() -> str:
    mounted = Path("/app/repo/gamer_import.xlsx")
    if mounted.parent.is_dir():
        return str(mounted)
    return str(_ROOT / "gamer_import.xlsx")
...
def main() -> None:
    p = argparse.ArgumentParser(description="Импорт users + payments из Excel (Fast Gamer / fastgamerbot).")
    p.add_argument(
        "xlsx",
        nargs="?",
        default=_default_xlsx_path(),
        help="Путь к .xlsx (по умолчанию ./gamer_import.xlsx или /app/repo/gamer_import.xlsx в Docker)",
    )
